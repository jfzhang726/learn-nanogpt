...
gradient_accumulation_steps = total_batch_size // (B*T) # = 32, how many batches to accumulate before doing backward pass
...
train_loader = DataLoaderLite(B=B, T=T) # train_loader = DataLoaderLite(B=16, T=1024) # GPT-2 max length 1024
...
for step in range(max_steps):
    ...
    print(f"step {step:4d} | lr: {lr:.4e}| loss {loss.item():6f} | accumulated loss: {loss_accum.item():6f} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms, tok/sec: {tokens_per_sec:.2f}")
